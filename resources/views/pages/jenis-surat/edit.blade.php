...
@section('container')
    ...
    <div class="main-container">
        ...
        <section class="section mb-4 p-8">
            <h2 class="text-xl font-semibold">Ubah Jenis Surat</h2>
            <hr class="my-4">
            <form action="/jenis_surat/{{ $jenis_surat->id }}" method="post">
                @method('put')
                @csrf
                <div class="mb-4">
                    <label for="jenis_surat" class="label">Nama jenis surat</label>
                    <input type="text" name="jenis_surat" id="jenis_surat" class="input-group @error('jenis_surat') is-invalid @enderror" value="{{ old('jenis_surat',$jenis_surat->jenis_surat) }}">
                    @error('jenis_surat')    
